...
async def update_cache(ca: str, *, mc, url, source="unknown", dex="", chain="", quote="", consensus=0.0,
                       image_url="", vol1h=None, **stats):
    """Seed the cache from a command-time lookup. ``stats`` are the extra
    TokenSnapshot fields (liquidity, 5m/1h trades and changes, pair age)."""
    ...
    async with TOKEN_CACHE_LOCK:
        token_cache[ca] = TokenSnapshot(
            ...
            consensus=consensus, delta=delta, image_url=image_url or "", vol1h=vol1h, **stats,
        )
